...
function renderizarTabelaConsumos() {
    ...
    try {
        ...
        pagina.forEach(c => {
            ...
            // Nome do gestor truncado
            const gestorNome = c.gestores_logictel?.nome || '-';
            const gestorTruncado = gestorNome.length > 12 ? gestorNome.substring(0, 10) + '…' : gestorNome;
            
            // Nome do projeto truncado
            const projetoNome = c.projetos?.nome || '-';
            const projetoTruncado = projetoNome.length > 10 ? projetoNome.substring(0, 8) + '…' : projetoNome;
            
            tbody.innerHTML += `
                <tr class="td-row">
                    <td class="coluna-sticky coluna-sticky-first" style="font-weight:600;font-size:10px;">${c.dc || '-'}</td>
                    <td style="max-width:80px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:10px;" title="${projetoNome}">${projetoTruncado}</td>
                    <td style="max-width:80px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:10px;" title="${gestorNome}">${gestorTruncado}</td>
                    <td><span class="tipo-badge ${tipoClass}">${tipoMed}</span></td>
                    <td style="font-size:10px;">${c.mes_apropriacao || '-'}</td>
                    <td style="font-size:10px;">${c.mes_medido || '-'}</td>
